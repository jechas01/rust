...
provide! { <'tcx> tcx, def_id, other, cdata,
    ...
    variances_of => { Arc::new(cdata.get_item_variances(def_id.index)) }
    ...
    mir_const_qualif => {
        (cdata.mir_const_qualif(def_id.index), Arc::new(IdxSetBuf::new_empty(0)))
    }
    typeck_tables_of => { cdata.item_body_tables(def_id.index, tcx) }
    closure_kind => { cdata.closure_kind(def_id.index) }
    fn_sig => { cdata.fn_sig(def_id.index, tcx) }
    inherent_impls => { Arc::new(cdata.get_inherent_implementations_for_type(def_id.index)) }
    ...
    dylib_dependency_formats => { Arc::new(cdata.get_dylib_dependency_formats()) }
    ...
    extern_crate => { Arc::new(cdata.extern_crate.get()) }
    is_no_builtins => { cdata.is_no_builtins(tcx.sess) }
    impl_defaultness => { cdata.get_impl_defaultness(def_id.index) }
    exported_symbol_ids => { Arc::new(cdata.get_exported_symbols()) }
    native_libraries => { Arc::new(cdata.get_native_libraries(tcx.sess)) }
    plugin_registrar_fn => {
    ...
    defined_lang_items => { Arc::new(cdata.get_lang_items()) }
    missing_lang_items => { Arc::new(cdata.get_missing_lang_items()) }
    ...
    used_crate_source => { Arc::new(cdata.source.clone()) }
...
pub fn provide_local<'tcx>(providers: &mut Providers<'tcx>) {
    ...
    *providers = Providers {
        ...
        native_libraries: |tcx, cnum| {
            ...
            Arc::new(native_libs::collect(tcx))
        },
        link_args: |tcx, cnum| {
            assert_eq!(cnum, LOCAL_CRATE);
            Arc::new(link_args::collect(tcx))
        },
...
impl CrateStore for cstore::CStore {
    ...
    fn load_macro_untracked(&self, id: DefId, sess: &Session) -> LoadedMacro {
        ...
        if let Some(ref proc_macros) = data.proc_macros {
            ...
            return LoadedMacro::ProcMacro(Arc::new(ext));
        }
