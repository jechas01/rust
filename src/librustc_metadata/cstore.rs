...
pub struct CrateMetadata {
    ...
    pub attribute_cache: Lock<[Vec<Option<Arc<[ast::Attribute]>>>; 2]>,
    ...
    /// compilation support.
    pub def_path_table: Arc<DefPathTable>,
    ...
    pub proc_macros: Option<Vec<(ast::Name, Arc<SyntaxExtension>)>>,
    // Foreign items imported from a dylib (Windows only)
    pub dllimport_foreign_items: FxHashSet<DefIndex>,
}

pub struct CStore {
    metas: RwLock<FxHashMap<CrateNum, Arc<CrateMetadata>>>,
...
impl CStore {
    ...
    pub fn get_crate_data(&self, cnum: CrateNum) -> Arc<CrateMetadata> {
        self.metas.borrow().get(&cnum).unwrap().clone()
    }

    pub fn set_crate_data(&self, cnum: CrateNum, data: Arc<CrateMetadata>) {
        self.metas.borrow_mut().insert(cnum, data);
    }

    pub fn iter_crate_data<I>(&self, mut i: I)
        where I: FnMut(CrateNum, &Arc<CrateMetadata>)
    {
