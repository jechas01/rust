...
pub struct TypeckTables<'tcx> {
    ...
    pub used_trait_imports: Arc<DefIdSet>,
...
impl<'tcx> TypeckTables<'tcx> {
    pub fn empty(local_id_root: Option<DefId>) -> TypeckTables<'tcx> {
        TypeckTables {
            ...
            used_trait_imports: Arc::new(DefIdSet()),
...
pub struct GlobalCtxt<'tcx> {
    ...
    trait_map: FxHashMap<DefIndex,
                         Arc<FxHashMap<ItemLocalId,
                                       Arc<StableVec<TraitCandidate>>>>>,

    /// Export map produced by name resolution.
    export_map: FxHashMap<DefId, Arc<Vec<Export>>>,
    ...
    // scratch every time.
    freevars: FxHashMap<DefId, Arc<Vec<hir::Freevar>>>,
    ...
    layout_interner: Lock<FxHashSet<&'tcx Layout>>,
...
fn _assert_gcx_sync<'tcx>() {
    use ::rustc_data_structures::lock::assert_sync;

    assert_sync::<&'tcx GlobalArenas<'tcx>>();
    assert_sync::<CtxtInterners<'tcx>>();
    //assert_sync::<&'tcx CrateStore>();
    assert_sync::<&'tcx Session>();
    assert_sync::<DepGraph>();
    assert_sync::<maps::OnDiskCache<'tcx>>();
    assert_sync::<CommonTypes<'tcx>>();
    assert_sync::<FxHashMap<DefIndex,
                         Arc<FxHashMap<ItemLocalId,
                                       Arc<StableVec<TraitCandidate>>>>>>();
    assert_sync::<FxHashMap<DefId, Arc<Vec<Export>>>>();
    assert_sync::<NamedRegionMap>();
    //assert_sync::<hir_map::Map<'tcx>>();
    //assert_sync::<maps::Maps<'tcx>>();
    assert_sync::<FxHashMap<DefId, Arc<Vec<hir::Freevar>>>>();
    assert_sync::<FxHashSet<DefId>>();
    assert_sync::<Vec<(DefId, Span)>>();
    assert_sync::<Lock<FxHashMap<ty::CReaderCacheKey, Ty<'tcx>>>>();
    assert_sync::<traits::SelectionCache<'tcx>>();
    assert_sync::<traits::EvaluationCache<'tcx>>();
    assert_sync::<Symbol>();
    assert_sync::<TargetDataLayout>();
    assert_sync::<Lock<NodeMap<Symbol>>>();
    assert_sync::<Lock<FxHashSet<&'tcx attr::Stability>>>();
    assert_sync::<Lock<FxHashSet<&'tcx Layout>>>();
    assert_sync::<Lock<Option<Vec<DefId>>>>();
    assert_sync::<Lock<mpsc::Sender<Box<Any + Send>>>>();
    assert_sync::<Arc<OutputFilenames>>();
}
...
impl<'a, 'gcx, 'tcx> TyCtxt<'a, 'gcx, 'tcx> {
                                  ...
                                  where F: for<'b> FnOnce(TyCtxt<'b, 'tcx, 'tcx>) -> R
    {
        ...
        let def_path_hash_to_def_id = if s.opts.build_dep_graph() {
            let upstream_def_path_tables: Vec<(CrateNum, Arc<_>)> = cstore
                .crates_untracked()
                .iter()
        ...
        for (k, v) in resolutions.trait_map {
            ...
            let map = trait_map.entry(hir_id.owner)
                .or_insert_with(|| Arc::new(FxHashMap()));
            Arc::get_mut(map).unwrap()
                            .insert(hir_id.local_id,
                                    Arc::new(StableVec::new(v)));
        }
        ...
        for (k, v) in named_region_map.defs {
            ...
            let map = defs.entry(hir_id.owner)
                .or_insert_with(|| Arc::new(FxHashMap()));
            Arc::get_mut(map).unwrap().insert(hir_id.local_id, v);
        }
        let mut late_bound = FxHashMap();
        for k in named_region_map.late_bound {
            let hir_id = hir.node_to_hir_id(k);
            let map = late_bound.entry(hir_id.owner)
                .or_insert_with(|| Arc::new(FxHashSet()));
            Arc::get_mut(map).unwrap().insert(hir_id.local_id);
        }
        ...
        for (k, v) in named_region_map.object_lifetime_defaults {
            ...
            let map = object_lifetime_defaults.entry(hir_id.owner)
                .or_insert_with(|| Arc::new(FxHashMap()));
            Arc::get_mut(map).unwrap().insert(hir_id.local_id, Arc::new(v));
        }

        //::rustc_data_structures::lock::assert_sync::<GlobalCtxt>();
    ...
    pub fn lang_items(self) -> Arc<middle::lang_items::LanguageItems> {
        self.get_lang_items(LOCAL_CRATE)
    }

    pub fn stability(self) -> Arc<stability::Index<'tcx>> {
    ...
    pub fn crates(self) -> Arc<Vec<CrateNum>> {
    ...
    pub fn crate_data_as_rc_any(self, cnum: CrateNum) -> Arc<Any> {
...
impl<'a, 'gcx, 'tcx> TyCtxt<'a, 'gcx, 'tcx> {
    ...
    pub fn in_scope_traits(self, id: HirId) -> Option<Arc<StableVec<TraitCandidate>>> {
    ...
    pub fn object_lifetime_defaults(self, id: HirId)
        -> Option<Arc<Vec<ObjectLifetimeDefault>>>
    {
...
struct NamedRegionMap {
    defs: FxHashMap<DefIndex, Arc<FxHashMap<ItemLocalId, resolve_lifetime::Region>>>,
    late_bound: FxHashMap<DefIndex, Arc<FxHashSet<ItemLocalId>>>,
    object_lifetime_defaults:
        FxHashMap<
            DefIndex,
            Arc<FxHashMap<ItemLocalId, Arc<Vec<ObjectLifetimeDefault>>>>,
        >,
}
...
pub fn provide(providers: &mut ty::maps::Providers) {
    ...
    providers.get_lang_items = |tcx, id| {
        ...
        // remove this because while the crate changes often the lint level map
        // will change rarely.
        tcx.dep_graph.with_ignore(|| Arc::new(middle::lang_items::collect(tcx)))
    };
    ...
    providers.stability_index = |tcx, cnum| {
        assert_eq!(cnum, LOCAL_CRATE);
        Arc::new(stability::Index::new(tcx))
    };
    ...
    providers.all_crate_nums = |tcx, cnum| {
        assert_eq!(cnum, LOCAL_CRATE);
        Arc::new(tcx.cstore.crates_untracked())
    };
    providers.postorder_cnums = |tcx, cnum| {
        assert_eq!(cnum, LOCAL_CRATE);
        Arc::new(tcx.cstore.postorder_cnums_untracked())
    };
