...
use rustc_data_structures::lock::{RwLock, Lock, LockCell, ReadGuardRef};
...
impl Session {
    ...
    pub fn incr_comp_session_dir(&self) -> ReadGuardRef<IncrCompSession, PathBuf> {
        let incr_comp_session = self.incr_comp_session.borrow();
        ReadGuardRef::new(incr_comp_session).map(|incr_comp_session| {
            match *incr_comp_session {
    ...
    pub fn incr_comp_session_dir_opt(&self) -> Option<ReadGuardRef<IncrCompSession, PathBuf>> {
...
pub fn build_session(sopts: config::Options,
    ...
    build_session_with_codemap(sopts,
                               ...
                               registry,
                               Arc::new(codemap::CodeMap::new(file_path_mapping)),
                               None)
}
...
pub fn build_session_with_codemap(sopts: config::Options,
    ...
    let emitter: Box<Emitter + Send> = match (sopts.error_format, emitter_dest) {
...
pub fn build_session_(sopts: config::Options,
    ...
    //::rustc_data_structures::lock::assert_sync::<Session>();
...
pub fn early_error(output: config::ErrorOutputType, msg: &str) -> ! {
    let emitter: Box<Emitter + Send> = match output {
