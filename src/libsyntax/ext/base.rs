...
pub trait Resolver {
    ...
    fn add_builtin(&mut self, ident: ast::Ident, ext: Arc<SyntaxExtension>);

    fn resolve_imports(&mut self);
    // Resolves attribute and derive legacy macros from `#![plugin(..)]`.
    fn find_legacy_attr_invoc(&mut self, attrs: &mut Vec<Attribute>) -> Option<Attribute>;
    fn resolve_invoc(&mut self, invoc: &mut Invocation, scope: Mark, force: bool)
                     -> Result<Option<Arc<SyntaxExtension>>, Determinacy>;
    fn resolve_macro(&mut self, scope: Mark, path: &ast::Path, kind: MacroKind, force: bool)
                     -> Result<Arc<SyntaxExtension>, Determinacy>;
...
impl Resolver for DummyResolver {
    ...
    fn add_builtin(&mut self, _ident: ast::Ident, _ext: Arc<SyntaxExtension>) {}
    ...
    fn resolve_macro(&mut self, _scope: Mark, _path: &ast::Path, _kind: MacroKind,
                     _force: bool) -> Result<Arc<SyntaxExtension>, Determinacy> {
        Err(Determinacy::Determined)
    }
    fn check_unused_macros(&self) {}
}
