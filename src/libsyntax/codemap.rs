...
use rustc_data_structures::lock::{Lock, LockGuard};
use std::hash::Hash;
...
pub struct CodeMap {
    pub(super) files: Lock<Vec<Arc<FileMap>>>,
    file_loader: Box<FileLoader + Sync + Send>,
    // This is used to apply the file path remapping as specified via
    // -Zremap-path-prefix to all FileMaps allocated within this CodeMap.
    path_mapping: FilePathMapping,
    stable_id_to_filemap: Lock<FxHashMap<StableFilemapId, Arc<FileMap>>>,
}
...
impl CodeMap {
    ...
    pub fn load_file(&self, path: &Path) -> io::Result<Arc<FileMap>> {
        let src = self.file_loader.read_file(path)?;
        Ok(self.new_filemap(path.to_str().unwrap().to_string(), src))
    }

    pub fn files(&self) -> LockGuard<Vec<Arc<FileMap>>> {
        self.files.borrow()
    }

    pub fn filemap_by_stable_id(&self, stable_id: StableFilemapId) -> Option<Arc<FileMap>> {
    ...
    pub fn new_filemap(&self, filename: FileName, src: String) -> Arc<FileMap> {
        ...
        let filemap = Arc::new(FileMap::new(
            filename,
            was_remapped,
            unmapped_path,
            src,
    ...
    pub fn new_filemap_and_lines(&self, filename: &str, src: &str) -> Arc<FileMap> {
    ...
    fn lookup_line(&self, pos: BytePos) -> Result<FileMapAndLine, Arc<FileMap>> {
    ...
    pub fn get_filemap(&self, filename: &str) -> Option<Arc<FileMap>> {
...
impl CodeMapper for CodeMap {
    ...
    fn ensure_filemap_source_present(&self, file_map: Arc<FileMap>) -> bool {
